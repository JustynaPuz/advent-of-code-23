        ...
        for (String line : lines) {
            ...
            while (!containsListWithOnlyZeros(difference)) {
                ...
                for (int i = 0; i < difference.get(index).size() - 1; i++) {
                    differenceForLine.add(difference.get(index).get(i + 1)
                            - difference.get(index).get(i));
                }
                difference.add(differenceForLine);
                index++;
            }
            difference.get(index).add(0, 0);
            for (int i = index - 1; i >= 0; i--) {
                difference.get(i).add(0, difference.get(i).get(0)
                        - difference.get(i + 1).get(0));
            }
            sum += difference.get(0).get(0);
        }
